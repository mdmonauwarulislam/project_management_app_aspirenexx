...
export default function Column({ column }) {
  ...
  return (
    <div className="w-full gap-4 p-6 mt-4 bg-white rounded-md" ref={dropRef} style={{
      background: isOver && '#eee'
    }}>
      <div className="">
        <div className="md:text-[22px] text-14px font-bold">{column.label} <span>{column.emoji}</span></div>
        <span className=" md:text-[22px] text-14px font-bold">
          {columnTasks?.length}
        </span>
      </div>
      <Tasks
